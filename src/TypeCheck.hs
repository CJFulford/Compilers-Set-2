...
checkArgs::[M_type] -> [M_type] -> Int ->  String
checkArgs [] [] _ = []
checkArgs (arg1:args) (parm1:parms) i =
    if (arg1 == parm1)
    then (checkArgs args parms (i+1))
    else ("mismatched arguments at position " ++ show i)
            ...
            case (checkExprs exprList st) of 
                ...
                Right ei ->
                    let
                        types    = map fst ei
                        exprs    = map snd ei
                        argsRes  = checkArgs types args 0
                    in
                    if (argsRes == [])
                    then Right (retType, I_app(I_call(label, level), exprs))
                    else Left ("Argument types dont match function parameters for " ++ argsRes)
        M_add -> 
            case (checkExprs exprList st) of 
                Left err -> Left err
                Right ei ->
                    ...
                    else Left ("Invalid Expression in add at " ++ show exprs ++ ". Only Integers are valid")
        M_sub -> 
            case (checkExprs exprList st) of 
                Left err -> Left err
                Right ei ->
                    ...
                    else Left ("Invalid Expression in sub at " ++ show exprs ++ ". Only Integers are valid")
        M_mul -> 
            case (checkExprs exprList st) of 
                Left err -> Left err
                Right ei ->
                    ...
                    else Left ("Invalid Expression in mul at " ++ show exprs ++ ". Only Integers are valid")
        M_div -> 
            case (checkExprs exprList st) of 
                Left err -> Left err
                Right ei ->
                    ...
                    else Left ("Invalid Expression in div at " ++ show exprs ++ ". Only Integers are valid")
        M_neg -> 
            case (checkExprs exprList st) of 
                Left err -> Left err
                Right ei ->
                    ...
                    else Left ("Invalid Expression in neg at " ++ show exprs ++ ". Only Integers are valid")
        M_lt -> 
            case (checkExprs exprList st) of 
                Left err -> Left err
                Right ei ->
                    ...
                    else Left ("Invalid Expression in < at " ++ show exprs ++ ". Only Integers are valid")
        M_le ->
            case (checkExprs exprList st) of 
                Left err -> Left err
                Right ei ->
        ...
        M_not -> 
            case (checkExprs exprList st) of 
                ...
                Right ei ->
                    ...
                    else Left ("Invalid Expression in not at " ++ show exprs ++ ". Only Integers are valid")
        M_and -> 
            case (checkExprs exprList st) of 
                Left err -> Left err
                Right ei ->
                    ...
                    else Left ("Invalid Expression in and at " ++ show exprs ++ ". Only Integers are valid")
        M_or -> 
            case (checkExprs exprList st) of 
                Left err -> Left err
                Right ei ->
                    ...
                    else Left ("Invalid Expression in of at " ++ show exprs ++ ". Only Integers are valid")
        ...
        case (checkExpr expr st) of
            ...
            Right (eType, iExpr)-> do
                ...
                then 
                    ...
                    Left ("Illegal Expression. Boolean expected at " ++ show iExpr)
    ...
    (M_cond(expr, stmt1, stmt2)) -> 
        case (checkExpr expr st) of
                ...
                Right (eType, iExpr)-> do
                    ...
                    else Left ("Illegal Expression. Boolean expected at " ++ show iExpr)
    -- M_read    String
    -- I_read_I  (Int,Int)
    -- I_read_B  (Int,Int)
    (M_read(str)) -> 
